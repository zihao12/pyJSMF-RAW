...
for i in range(n_sample):
    ...
    ## the following only makes sense when document is equal length
    d_ = X.sum(axis = 1)[0]
    d2 = d_ * (d_ - 1)
    #pdb.set_trace()
    correc = (X @ u)[:, None] * X - X * u[None, :]
    correc = (correc / d2)**2
    correc = correc.mean(axis = 0) / n

    est[:, i] =  (est_notcorrected[:, i] - correc) / (1 - 1/n)

    
outputfile="findK-exper-step2-2-constd.pkl"

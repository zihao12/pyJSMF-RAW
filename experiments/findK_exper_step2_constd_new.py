...
for i in range(n_sample):
    ...
    C_ = compute_C_unbiased_local(X.copy())
    Cbar_ = C_ / C_.sum(axis = 1)[:, None]

    # dg = (Cbar_ @ u)**2
    dg = (C_ @ u)**2 / (w**2)
    est_notcorrected[:, i]  = dg
    est[:, i] = dg - compute_bias_stepk(X.copy(), u.copy(), d.copy(), w.copy())
    se[:, i] = compute_se_stepk(X.copy(), u.copy(), C_.copy(), d.copy(), w.copy())
    
    # est[:, i] = dg - compute_bias_stepk(X.copy(), u.copy(), d.copy(), w_true.copy())
    # se[:, i] = compute_se_stepk(X.copy(), u.copy(), C.copy(), d.copy(), w_true.copy()) ## cheating
    
outputfile="findK-exper-step2-constd-new.pkl"
with open(outputfile, "wb") as f:
    pickle.dump({'est':est, 'est_notcorrected':est_notcorrected, 'truth':truth, 'se':se,
        'C':C, 'S0':S0, 'u':u}, f)

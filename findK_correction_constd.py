...
def findS_correction(X, Cbar, C, K, const):
		n, p = X.shape
		d = X.sum() / n
		w = X.sum(axis = 0)
		w /= w.sum()
...
def compute_se_step1(X, C, d, w):
	...
	var =  * mu4 + 4*n*(n-1)*mu4_mu + (2*n*n - 3*n)*mu2_2 + 4*n*(n-1)*(n-3)*mu2_mu_2 - 2*n*(n-1)*(2*n - 3)*mu_4
	## compute covariance
	cov = 2 * (X3 * (X2.sum(axis = 1) * X.sum(axis = 1) - X3.sum(axis = 1))).mean(axis = 0) / np.power(D, 3)
	out = (var + cov) / np.power(n, 4)

	return np.sqrt(out / np.power(w, 4))


def correction_step2(dg, X, u, C, d, w):
	dg -= compute_bias_stepk(X, u, d, w)
	se = compute_se_step2(X, u, C, d, w)
	dg = shrink(dg, se)
...
def compute_se_step2(X, u, C, d, w):
	n = X.shape[0]
	D = d * (d-1)
	mu = C @ u
	...
	out = n * mu4 + 4*n*(n-1)*mu4*mu + (2*n*n - 3*n)*np.power(mu2, 2) + 4*n*(n-1)*(n-3)*mu2*np.power(mu, 2) - 2*n*(n-1)*(2*n - 3)*np.power(mu, 4)
